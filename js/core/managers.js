...
class AuthManager {
    ...
    static bindUI() {
        ...
        Utils.$('btn-do-login').onclick = async () => {
            ...
            if (!email || !pass) return Utils.toast('Р—Р°РїРѕР»РЅРёС‚Рµ РІСЃРµ РїРѕР»СЏ', 'error');
            try {
                Utils.$('btn-do-login').disabled = true;
                await signInWithEmailAndPassword(auth, email, pass);
            } catch (e) {
                Utils.toast('РћС€РёР±РєР° РІС…РѕРґР°. РџСЂРѕРІРµСЂСЊС‚Рµ РґР°РЅРЅС‹Рµ.', 'error');
        ...
        Utils.$('btn-do-reg').onclick = async () => {
            ...
            if (!email || pass.length < 6 || !name || !username) return Utils.toast('Р—Р°РїРѕР»РЅРёС‚Рµ РїРѕР»СЏ. РџР°СЂРѕР»СЊ РѕС‚ 6 СЃРёРјРІРѕР»РѕРІ.', 'error');
            if (!/^[a-z0-9_]{3,15}$/.test(username)) return Utils.toast('ID: 3-15 СЃРёРјРІРѕР»РѕРІ, С‚РѕР»СЊРєРѕ a-z, 0-9 Рё _', 'error');

            try {
                Utils.$('btn-do-reg').disabled = true;
                
                const isAvail = await ProfileManager.checkUsernameAvailability(username);
                if (!isAvail) throw new Error('Р­С‚РѕС‚ @ID СѓР¶Рµ Р·Р°РЅСЏС‚ РґСЂСѓРіРёРј РїРѕР»СЊР·РѕРІР°С‚РµР»РµРј!');
        ...
        const handleGoogleAuth = async () => {
            ...
            } catch (e) { Utils.toast('РћС€РёР±РєР° РІС…РѕРґР° С‡РµСЂРµР· Google', 'error'); }
        };
...
class ProfileManager {
    static getRoleBadgeHtml(profile, uid = null) {
        if (!profile) return '';
        if (AdminPanel.isCreatorProfile(profile, uid)) return `<span class="role-badge badge-creator">РЎРѕР·РґР°С‚РµР»СЊ</span>`;
        if (AdminPanel.isModeratorProfile(profile, uid)) return `<span class="role-badge badge-moderator">РњРѕРґРµСЂР°С‚РѕСЂ</span>`;
        return '';
    }
    ...
    static async createProfile(uid, name, username, email) {
        ...
        if (isDeveloperProfile && developerUid && developerUid !== uid) {
            throw new Error('ID developer Р·Р°СЂРµР·РµСЂРІРёСЂРѕРІР°РЅ');
        }
    ...
    static openEditProfileModal() {
        ...
        Utils.$('btn-save-profile').onclick = async () => {
            ...
            try {
                ...
                Utils.toast('РџСЂРѕС„РёР»СЊ СЃРѕС…СЂР°РЅРµРЅ');
    ...
    static async saveProfile() {
        ...
        if (!name || !username) throw new Error('РРјСЏ Рё ID РѕР±СЏР·Р°С‚РµР»СЊРЅС‹');
        if (!/^[a-z0-9_]{3,15}$/.test(username)) throw new Error('ID: 3-15 СЃРёРјРІРѕР»РѕРІ, a-z, 0-9, _');
        ...
        if (isCreatorProfile && username !== oldProfile.username) throw new Error('ID РЎРѕР·РґР°С‚РµР»СЏ РЅРµР»СЊР·СЏ РёР·РјРµРЅРёС‚СЊ');

        const updates = {};
        
        if (username !== oldProfile.username) {
            const snap = await get(ref(db, `usernames/${username}`));
            if (snap.exists() && snap.val() !== uid) throw new Error('Р­С‚РѕС‚ ID СѓР¶Рµ Р·Р°РЅСЏС‚');
    ...
    static async openViewProfileModal(targetUid) {
        ...
        if (!profile) return Utils.toast('РџРѕР»СЊР·РѕРІР°С‚РµР»СЊ РЅРµ РЅР°Р№РґРµРЅ', 'error');
        ...
        const joinDate = profile.createdAt ? new Date(profile.createdAt).toLocaleDateString() : 'РќРµРёР·РІРµСЃС‚РЅРѕ';
        ...
        Utils.$('view-bio').innerHTML = `
            ${Utils.escapeHtml(profile.bio || 'РџРѕР»СЊР·РѕРІР°С‚РµР»СЊ РЅРµ РґРѕР±Р°РІРёР» РѕРїРёСЃР°РЅРёРµ.')}<br><br>
            <strong style="color:var(--text-main);">РЎС‚Р°С‚РёСЃС‚РёРєР°:</strong><br>
            Р”СЂСѓР·РµР№: ${friendsCount}<br>
            РќР° РїР»Р°С‚С„РѕСЂРјРµ СЃ: ${joinDate}
        `;
        ...
        if (targetUid === AppState.currentUser.uid) {
            ...
            if (isFriend) {
                actionBtn.innerText = 'РќР°РїРёСЃР°С‚СЊ СЃРѕРѕР±С‰РµРЅРёРµ';
                actionBtn.onclick = () => {
                    Utils.$('modal-view-profile').classList.remove('active');
                    DirectMessages.openChat(targetUid, profile.name);
                };
            } else {
                actionBtn.innerText = 'Р”РѕР±Р°РІРёС‚СЊ РІ РґСЂСѓР·СЊСЏ';
                actionBtn.onclick = () => {
...
// ============================================================================
// 4. РЎРРЎРўР•РњРђ Р”Р РЈР—Р•Р™ Р Р›РР§РќР«РҐ РЎРћРћР‘Р©Р•РќРР™ (СЃ Share Room)
// ============================================================================
...
class FriendsManager {
    ...
    static async sendFriendRequest(targetUid) {
        ...
        try {
            ...
            Utils.toast('Р—Р°СЏРІРєР° РѕС‚РїСЂР°РІР»РµРЅР°');
        } catch (e) { Utils.toast('РћС€РёР±РєР° РѕС‚РїСЂР°РІРєРё', 'error'); }
    }
    ...
    static async handleRequest(targetUid, accept) {
        ...
        try {
            ...
            await update(ref(db), updates);
            Utils.toast(accept ? 'Р”СЂСѓРі РґРѕР±Р°РІР»РµРЅ' : 'Р—Р°СЏРІРєР° РѕС‚РєР»РѕРЅРµРЅР°');
        } catch (e) { Utils.toast('РћС€РёР±РєР°', 'error'); }
    }
    ...
    static async renderRequests(requests) {
        ...
        if (keys.length > 0) {
            ...
            container.innerHTML = '<div style="font-size: 12px; color: var(--text-muted); padding: 5px; text-align: center;">РќРµС‚ РЅРѕРІС‹С… Р·Р°СЏРІРѕРє</div>';
            return;
        }
        ...
        for (const uid of keys) {
            ...
            div.innerHTML = `
                <div style="font-size: 13px;"><strong>${Utils.escapeHtml(profile.name)}</strong> С…РѕС‡РµС‚ РІ РґСЂСѓР·СЊСЏ</div>
                <div class="req-actions">
                    <button class="btn-small btn-accept">РџСЂРёРЅСЏС‚СЊ</button>
                    <button class="btn-small btn-decline">РћС‚РєР»РѕРЅРёС‚СЊ</button>
                </div>
            `;
    ...
    static async renderFriends(friendsMap) {
        ...
        if (keys.length === 0) {
            container.innerHTML = '<div style="font-size: 12px; color: var(--text-muted); padding: 5px; text-align: center;">РќРµС‚ РґСЂСѓР·РµР№. РћР±С‰Р°Р№С‚РµСЃСЊ РІ РєРѕРјРЅР°С‚Р°С…!</div>';
            return;
        }
                ...
                div.innerHTML = `
                    ...
                    <div class="friend-info-col">
                        ...
                        <div class="friend-status">
                            ...
                            ${isOnline ? 'РћРЅР»Р°Р№РЅ' : 'РћС„Р»Р°Р№РЅ'}
                        </div>
                    </div>
                `;
            });
...
class DirectMessages {
    ...
    static closeChat() {
        ...
        if (Utils.$('dm-chat-title')) Utils.$('dm-chat-title').innerText = 'Р›РёС‡РЅС‹Р№ С‡Р°С‚';
    }
        ...
        const unsub = onValue(dmRoot, (snap) => {
            ...
            Object.entries(chats).forEach(([chatId, chat]) => {
                ...
                if (chat.lastMessage.type === 'invite') {
                    Utils.toast(`Р›РЎ: ${chat.lastMessage.fromName} РїСЂРёРіР»Р°С€Р°РµС‚ РІР°СЃ РІ РєРѕРјРЅР°С‚Сѓ!`);
                } else {
                    Utils.toast(`Р›РЎ РѕС‚ ${chat.lastMessage.fromName}: ${chat.lastMessage.text}`);
                }
            });
        });
    ...
    static openChat(targetUid, targetName) {
        ...
        Utils.$('dm-chat-title').innerText = `Р§Р°С‚: ${targetName}`;
    ...
    static renderMessages(messages) {
        ...
        if (!messages.length) { list.innerHTML = '<div style="color:var(--text-muted); text-align:center; padding:20px;">РќРµС‚ СЃРѕРѕР±С‰РµРЅРёР№</div>'; return; }
            ...
            if (m.type === 'invite') {
                return `
                    <div class="m-line ${isSelf ? 'self' : ''}">
                        <strong>${Utils.escapeHtml(isSelf ? 'Р’С‹' : m.fromName)}</strong>
                        <div class="bubble" style="border: 1px solid var(--accent); background: rgba(46,213,115,0.1);">
                            <div style="font-weight:bold; margin-bottom:5px;">РџСЂРёРІРµС‚! Р—Р°С…РѕРґРё Рє РЅР°Рј:</div>
                            <div style="font-size: 16px;">рџ“є ${Utils.escapeHtml(m.roomName)}</div>
                            <div style="font-size: 12px; opacity:0.8; margin-bottom:8px;">рџ‘Ґ Р—СЂРёС‚РµР»РµР№: ${m.membersCount || 1}</div>
                            ${!isSelf ? `
                                <div style="display:flex; gap:10px;">
                                    <button class="primary-btn" style="padding:6px; font-size:12px; width:auto;" onclick="window.acceptRoomInvite('${m.roomId}')">РџСЂРёРЅСЏС‚СЊ</button>
                                    <button class="secondary-btn" style="padding:6px; font-size:12px; width:auto;" onclick="this.parentElement.innerHTML='РћС‚РєР»РѕРЅРµРЅРѕ'">РћС‚РєР»РѕРЅРёС‚СЊ</button>
                                </div>
                            ` : `<div style="font-size:11px; opacity:0.6; margin-top:5px;">РџСЂРёРіР»Р°С€РµРЅРёРµ РѕС‚РїСЂР°РІР»РµРЅРѕ</div>`}
                        </div>
                    </div>
                `;
            }
    ...
    static async sendRoomInvite(targetUid) {
        ...
        if (AppState.currentPresenceCache?.[targetUid]) return Utils.toast('РџРѕР»СЊР·РѕРІР°С‚РµР»СЊ СѓР¶Рµ РЅР°С…РѕРґРёС‚СЃСЏ РІ РєРѕРјРЅР°С‚Рµ', 'error');

        const roomData = AppState.roomsCache.get(AppState.currentRoomId);
        if (!roomData) return Utils.toast('РљРѕРјРЅР°С‚Р° Р±РѕР»СЊС€Рµ РЅРµ СЃСѓС‰РµСЃС‚РІСѓРµС‚', 'error');

        const cooldownKey = `${AppState.currentRoomId}:${targetUid}`;
        const lastInviteTs = AppState.inviteCooldowns.get(cooldownKey) || 0;
        if (Date.now() - lastInviteTs < 10000) return Utils.toast('РќРµ СЃРїР°РјСЊС‚Рµ РёРЅРІР°Р№С‚Р°РјРё вЂ” РїРѕРґРѕР¶РґРёС‚Рµ 10 СЃРµРєСѓРЅРґ', 'error');
        ...
        const payload = { 
            ...
            fromName: senderProfile.name || AppState.currentUser.displayName || 'РџРѕР»СЊР·РѕРІР°С‚РµР»СЊ', 
            text: `РџСЂРёРіР»Р°С€РµРЅРёРµ РІ РєРѕРјРЅР°С‚Сѓ: ${roomData.name}`,
            ts: Date.now() 
        };
        ...
        await push(ref(db, `direct-messages/${chatId}/messages`), payload);
        Utils.toast('РџСЂРёРіР»Р°С€РµРЅРёРµ РѕС‚РїСЂР°РІР»РµРЅРѕ');
    }
}
...
window.acceptRoomInvite = async (roomId) => {
    ...
    try {
        ...
        if (!snap.exists()) return Utils.toast('РљРѕРјРЅР°С‚Р° Р±РѕР»СЊС€Рµ РЅРµ СЃСѓС‰РµСЃС‚РІСѓРµС‚', 'error');
        ...
        if (AppState.currentRoomId === roomId) {
            ...
            return Utils.toast('Р’С‹ СѓР¶Рµ РЅР°С…РѕРґРёС‚РµСЃСЊ РІ СЌС‚РѕР№ РєРѕРјРЅР°С‚Рµ');
        }

        DirectMessages.closeChat();
        RoomManager.enterRoomFinal(roomId, roomData); 
    } catch (e) {
        Utils.toast('РќРµ СѓРґР°Р»РѕСЃСЊ РѕС‚РєСЂС‹С‚СЊ РїСЂРёРіР»Р°С€РµРЅРёРµ', 'error');
    }
};

// ============================================================================
// 5. РђР”РњРРќ-РџРђРќР•Р›Р¬ Р Р“Р›РћР‘РђР›Р¬РќРћР• РЈРџР РђР’Р›Р•РќРР• (РЎ Р РћР›РЇРњР)
// ============================================================================
...
class AdminPanel {
    ...
    static requireAdmin() {
        if (!AppState.currentUser || !this.isCurrentUserAdmin()) {
            Utils.toast('РќРµРґРѕСЃС‚Р°С‚РѕС‡РЅРѕ РїСЂР°РІ РґР»СЏ Р°РґРјРёРЅ-РґРµР№СЃС‚РІРёСЏ', 'error');
            return false;
        }
        return true;
    }

    // Р—Р°С‰РёС‚Р°: РњРѕРґРµСЂР°С‚РѕСЂ РЅРµ РјРѕР¶РµС‚ С‚СЂРѕРіР°С‚СЊ РЎРѕР·РґР°С‚РµР»СЏ
    static async checkModRestrictionsForTarget(targetUid) {
        ...
        if (await this.isProtectedCreatorTarget(targetUid)) {
            Utils.toast('РњРѕРґРµСЂР°С‚РѕСЂ РЅРµ РјРѕР¶РµС‚ РІР·Р°РёРјРѕРґРµР№СЃС‚РІРѕРІР°С‚СЊ СЃ РїСЂРѕС„РёР»РµРј РЎРѕР·РґР°С‚РµР»СЏ', 'error');
            return false;
        }
        return true;
    }

    // Р—Р°С‰РёС‚Р°: РњРѕРґРµСЂР°С‚РѕСЂ РЅРµ РјРѕР¶РµС‚ С‚СЂРѕРіР°С‚СЊ РєРѕРјРЅР°С‚Сѓ РЎРѕР·РґР°С‚РµР»СЏ (РёР»Рё РєРѕРјРЅР°С‚Сѓ, РіРґРµ РѕРЅ СЃРёРґРёС‚)
    static async checkModRestrictionsForRoom(roomId) {
        if (this.isCurrentUserCreator()) return true;
        if (await this.isProtectedCreatorRoom(roomId)) {
            Utils.toast('РЈ РјРѕРґРµСЂР°С‚РѕСЂР° РЅРµС‚ РїСЂР°РІ РЅР° СЌС‚Сѓ РєРѕРјРЅР°С‚Сѓ (РїСЂРёРЅР°РґР»РµР¶РёС‚ РёР»Рё Р·Р°РЅСЏС‚Р° РЎРѕР·РґР°С‚РµР»РµРј)', 'error');
            return false;
        }
        return true;
    }
    ...
    static ensureUI() {
        ...
        modal.innerHTML = `
            <div class="modal-content glass-panel" style="width:min(1180px,100%); padding:22px;">
                <div style="display:flex; justify-content:space-between; align-items:center; gap:16px; margin-bottom:16px;">
                    <div>
                        <h2 style="margin:0;">РђРґРјРёРЅ-РїР°РЅРµР»СЊ</h2>
                        <div style="font-size:12px; color:var(--text-muted); margin-top:4px;">Р”РѕСЃС‚СѓРї РґР»СЏ РЎРѕР·РґР°С‚РµР»СЏ Рё РњРѕРґРµСЂР°С‚РѕСЂРѕРІ</div>
                    </div>
                    <button class="secondary-btn" id="btn-close-admin-panel" style="width:auto; padding:8px 12px;">вњ•</button>
                </div>
                ...
                <div style="border:1px solid var(--border-light); border-radius:16px; padding:16px; background:rgba(255,255,255,0.02); margin-bottom: 16px;">
                    <div style="font-weight:700; margin-bottom:10px;">РЈРїСЂР°РІР»РµРЅРёРµ РїСЂР°РІР°РјРё (РўРѕР»СЊРєРѕ РґР»СЏ РЎРѕР·РґР°С‚РµР»СЏ)</div>
                    <div style="display:flex; gap:8px;">
                        <input type="text" id="admin-mod-username" placeholder="ID РїРѕР»СЊР·РѕРІР°С‚РµР»СЏ (Р±РµР· @)" style="margin:0; flex:1;">
                        <button class="primary-btn" id="btn-admin-grant-mod" style="width:auto; padding:0 16px;">РќР°Р·РЅР°С‡РёС‚СЊ РњРѕРґРµСЂР°С‚РѕСЂР°</button>
                        <button class="danger-btn" id="btn-admin-revoke-mod" style="width:auto; padding:0 16px;">РЎРЅСЏС‚СЊ РњРѕРґРµСЂР°С‚РѕСЂР°</button>
                    </div>
                </div>
                ...
                <div style="display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:16px; margin-bottom:16px;">
                    <div style="border:1px solid var(--border-light); border-radius:16px; padding:16px; background:rgba(255,255,255,0.02);">
                        <div style="font-weight:700; margin-bottom:10px;">Р“Р»РѕР±Р°Р»СЊРЅРѕРµ РѕРїРѕРІРµС‰РµРЅРёРµ</div>
                        <textarea id="admin-announcement-input" rows="4" placeholder="РЎРѕРѕР±С‰РµРЅРёРµ РґР»СЏ РІСЃРµС… РѕРЅР»Р°Р№РЅ-РїРѕР»СЊР·РѕРІР°С‚РµР»РµР№..."></textarea>
                        <div style="display:flex; gap:8px;">
                            <button class="primary-btn" id="btn-admin-send-announcement">Р Р°Р·РѕСЃР»Р°С‚СЊ</button>
                            <button class="secondary-btn" id="btn-admin-clear-announcement">РћС‡РёСЃС‚РёС‚СЊ</button>
                        </div>
                    </div>

                    <div style="border:1px solid var(--border-light); border-radius:16px; padding:16px; background:rgba(255,255,255,0.02);">
                        <div style="font-weight:700; margin-bottom:10px;">Р‘С‹СЃС‚СЂС‹Рµ РґРµР№СЃС‚РІРёСЏ</div>
                        <div style="display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:8px;">
                            <button class="danger-btn" id="btn-admin-delete-all-rooms">РЈРґР°Р»РёС‚СЊ РІСЃРµ РєРѕРјРЅР°С‚С‹</button>
                            <button class="secondary-btn" id="btn-admin-purge-empty-rooms">РћС‡РёСЃС‚РёС‚СЊ РїСѓСЃС‚С‹Рµ РєРѕРјРЅР°С‚С‹</button>
                            <button class="secondary-btn" id="btn-admin-clear-dms">РЈРґР°Р»РёС‚СЊ РІСЃРµ Р›РЎ</button>
                            <button class="secondary-btn" id="btn-admin-toggle-room-lock">Р‘Р»РѕРєРёСЂРѕРІР°С‚СЊ СЃРѕР·РґР°РЅРёРµ РєРѕРјРЅР°С‚</button>
                            <button class="secondary-btn" id="btn-admin-refresh">РћР±РЅРѕРІРёС‚СЊ РґР°РЅРЅС‹Рµ</button>
                            <button class="secondary-btn" id="btn-admin-clear-user-editor">РЎР±СЂРѕСЃРёС‚СЊ РІС‹Р±СЂР°РЅРЅРѕРіРѕ СЋР·РµСЂР°</button>
                        </div>
                    </div>
                </div>
                ...
                <div style="display:grid; grid-template-columns:1.15fr 0.85fr; gap:16px;">
                    <div style="display:flex; flex-direction:column; gap:16px; min-width:0;">
                        <div style="border:1px solid var(--border-light); border-radius:16px; padding:16px; background:rgba(255,255,255,0.02);">
                            <div style="display:flex; justify-content:space-between; align-items:center; gap:10px; margin-bottom:10px;">
                                <div style="font-weight:700;">РђРєС‚РёРІРЅС‹Рµ РєРѕРјРЅР°С‚С‹</div>
                                <div style="font-size:12px; color:var(--text-muted);">РЈРґР°Р»РµРЅРёРµ Р»СЋР±С‹С… РєРѕРјРЅР°С‚ РѕРґРЅРёРј РЅР°Р¶Р°С‚РёРµРј</div>
                            </div>
                        ...
                        <div style="border:1px solid var(--border-light); border-radius:16px; padding:16px; background:rgba(255,255,255,0.02);">
                            <div style="display:flex; justify-content:space-between; align-items:center; gap:10px; margin-bottom:10px;">
                                <div style="font-weight:700;">РћРЅР»Р°Р№РЅ РїРѕР»СЊР·РѕРІР°С‚РµР»Рё</div>
                                <div style="font-size:12px; color:var(--text-muted);">Р¤РѕСЂСЃ-РІС‹С…РѕРґ / РєРёРє РёР· РєРѕРјРЅР°С‚С‹</div>
                            </div>
                    ...
                    <div style="border:1px solid var(--border-light); border-radius:16px; padding:16px; background:rgba(255,255,255,0.02); min-width:0;">
                        <div style="font-weight:700; margin-bottom:10px;">РЈРїСЂР°РІР»РµРЅРёРµ РїРѕР»СЊР·РѕРІР°С‚РµР»СЏРјРё</div>
                        <div style="display:flex; gap:8px; margin-bottom:12px;">
                            <input type="text" id="admin-user-search" placeholder="РџРѕРёСЃРє РїРѕ @id РёР»Рё uid" style="margin:0;">
                            <button class="primary-btn" id="btn-admin-find-user" style="width:auto; padding:0 16px;">РќР°Р№С‚Рё</button>
                        </div>
                        ...
                        <div id="admin-user-editor" data-target-uid="" style="display:flex; flex-direction:column; gap:10px;">
                            <div style="font-size:13px; color:var(--text-muted); padding:12px; border:1px dashed var(--border-light); border-radius:12px;">
                                Р’С‹Р±РµСЂРёС‚Рµ РїРѕР»СЊР·РѕРІР°С‚РµР»СЏ С‡РµСЂРµР· РїРѕРёСЃРє РёР»Рё РєР»РёРє РїРѕ СЃРїРёСЃРєСѓ РѕРЅР»Р°Р№РЅР°.
                            </div>
                        </div>
                    </div>
                </div>
        ...
        // РњРѕРґРµСЂР°С‚РѕСЂСЃРєРёРµ РєРЅРѕРїРєРё
    ...
    static async toggleModRole(grant) {
        if (!this.isCurrentUserCreator()) return Utils.toast('РўРѕР»СЊРєРѕ РЎРѕР·РґР°С‚РµР»СЊ РјРѕР¶РµС‚ СѓРїСЂР°РІР»СЏС‚СЊ РјРѕРґРµСЂР°С‚РѕСЂР°РјРё', 'error');
        const username = Utils.$('admin-mod-username').value.trim().toLowerCase().replace('@', '');
        if (!username) return Utils.toast('Р’РІРµРґРёС‚Рµ ID РїРѕР»СЊР·РѕРІР°С‚РµР»СЏ', 'error');

        const snap = await get(ref(db, `usernames/${username}`));
        if (!snap.exists()) return Utils.toast('РџРѕР»СЊР·РѕРІР°С‚РµР»СЊ РЅРµ РЅР°Р№РґРµРЅ', 'error');
        const targetUid = snap.val();

        if (await this.isProtectedCreatorTarget(targetUid)) {
            return Utils.toast('РќРµР»СЊР·СЏ РёР·РјРµРЅРёС‚СЊ СЂРѕР»СЊ РЎРѕР·РґР°С‚РµР»СЏ', 'error');
        }

        await update(ref(db, `users/${targetUid}/profile`), { role: grant ? 'moderator' : null });
        Utils.toast(grant ? 'РџСЂР°РІР° РјРѕРґРµСЂР°С‚РѕСЂР° РІС‹РґР°РЅС‹' : 'РџСЂР°РІР° РјРѕРґРµСЂР°С‚РѕСЂР° СЃРЅСЏС‚С‹');
    ...
    static init() {
        ...
        const annUnsub = onValue(annRef, (snap) => {
            ...
            Utils.toast(`РћРїРѕРІРµС‰РµРЅРёРµ: ${payload.text}`);
        });
        ...
        const forceSignOutUnsub = onValue(forceSignOutRef, async (snap) => {
            ...
            if (!this.isCurrentUserAdmin()) {
                Utils.toast('РђРґРјРёРЅРёСЃС‚СЂР°С‚РѕСЂ Р·Р°РІРµСЂС€РёР» РІР°С€Сѓ СЃРµСЃСЃРёСЋ', 'error');
                await signOut(auth);
            }
        });
        ...
        const forceLeaveRoomUnsub = onValue(forceLeaveRoomRef, (snap) => {
            ...
            if (!this.isCurrentUserAdmin() && AppState.currentRoomId && (!payload.roomId || payload.roomId === AppState.currentRoomId)) {
                Utils.toast('РђРґРјРёРЅРёСЃС‚СЂР°С‚РѕСЂ СѓРґР°Р»РёР» РІР°СЃ РёР· РєРѕРјРЅР°С‚С‹', 'error');
                RoomManager.leaveRoom();
            }
        });
    ...
    static syncSidebarButton(profile = {}) {
        ...
        if (!btn) {
            ...
            btn.innerText = 'РђРґРјРёРЅ-РїР°РЅРµР»СЊ';
    ...
    static renderStats(stats) {
        const cards = [
            { label: 'Р’СЃРµРіРѕ РїРѕР»СЊР·РѕРІР°С‚РµР»РµР№', value: Object.keys(stats.usersData).length },
            { label: 'РћРЅР»Р°Р№РЅ СЃРµР№С‡Р°СЃ', value: stats.onlineUsers.length },
            { label: 'РђРєС‚РёРІРЅС‹С… РєРѕРјРЅР°С‚', value: stats.rooms.length },
            { label: 'РџСЂРёРІР°С‚РЅС‹С… РєРѕРјРЅР°С‚', value: stats.privateRooms.length },
            { label: 'РџСѓСЃС‚С‹С… РєРѕРјРЅР°С‚', value: stats.emptyRooms.length },
            { label: 'Р›РёС‡РЅС‹С… С‡Р°С‚РѕРІ', value: Object.keys(stats.dmData).length }
        ];
        ...
        if (lockBtn) lockBtn.innerText = AppState.admin.settings.roomCreationBlocked ? 'Р Р°Р·Р±Р»РѕРєРёСЂРѕРІР°С‚СЊ СЃРѕР·РґР°РЅРёРµ РєРѕРјРЅР°С‚' : 'Р‘Р»РѕРєРёСЂРѕРІР°С‚СЊ СЃРѕР·РґР°РЅРёРµ РєРѕРјРЅР°С‚';
    }
    ...
    static renderRoomsList(rooms) {
        ...
        if (!rooms.length) {
            list.innerHTML = `<div style="font-size:13px; color:var(--text-muted); padding:8px;">РќРµС‚ Р°РєС‚РёРІРЅС‹С… РєРѕРјРЅР°С‚</div>`;
            return;
        }
            ...
            return `
                <div style="border:1px solid var(--border-light); border-radius:12px; padding:12px; display:flex; justify-content:space-between; gap:10px; align-items:center; flex-wrap:wrap;">
                    <div style="min-width:0; flex:1;">
                        <div style="font-weight:700; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">${room.isPrivate ? 'рџ”’ ' : ''}${Utils.escapeHtml(room.name || 'Р‘РµР· РЅР°Р·РІР°РЅРёСЏ')}</div>
                        <div style="font-size:12px; color:var(--text-muted); margin-top:4px;">ID: ${roomId} вЂў рџ‘Ґ ${membersCount} вЂў РҐРѕСЃС‚: ${Utils.escapeHtml(room.hostName || 'РќРµРёР·РІРµСЃС‚РЅРѕ')}</div>
                    </div>
                    <div style="display:flex; gap:8px;">
                        <button class="secondary-btn admin-enter-room-btn" data-room-id="${roomId}" style="width:auto; padding:8px 12px;">Р’РѕР№С‚Рё</button>
                        <button class="danger-btn admin-delete-room-btn" data-room-id="${roomId}" style="width:auto; padding:8px 12px;">Р—Р°РєСЂС‹С‚СЊ</button>
                    </div>
                </div>
            `;
        }).join('');
        ...
        list.querySelectorAll('.admin-enter-room-btn').forEach(btn => {
            btn.onclick = () => {
                ...
                if (!roomData) return Utils.toast('РљРѕРјРЅР°С‚Р° СѓР¶Рµ СѓРґР°Р»РµРЅР°', 'error');
                Utils.$('modal-admin-panel').classList.remove('active');
                RoomManager.enterRoomFinal(roomId, roomData);
            };
        });
    ...
    static renderOnlineUsers(usersData) {
        ...
        if (!onlineEntries.length) {
            list.innerHTML = `<div style="font-size:13px; color:var(--text-muted); padding:8px;">РЎРµР№С‡Р°СЃ РЅРёРєС‚Рѕ РЅРµ РѕРЅР»Р°Р№РЅ</div>`;
            return;
        }
        ...
        list.innerHTML = onlineEntries.map(([uid, userData]) => {
            ...
            return `
                <div style="border:1px solid var(--border-light); border-radius:12px; padding:12px; display:flex; justify-content:space-between; gap:10px; align-items:center; flex-wrap:wrap;">
                    <div style="min-width:0; flex:1;">
                        <div style="font-weight:700;">${Utils.escapeHtml(profile.name || 'Р‘РµР· РёРјРµРЅРё')} <span style="color:var(--accent); font-size:12px;">@${Utils.escapeHtml(profile.username || uid)}</span></div>
                        <div style="font-size:12px; color:var(--text-muted); margin-top:4px;">
                            UID: ${uid}${roomMeta ? ` вЂў Р’ РєРѕРјРЅР°С‚Рµ: ${Utils.escapeHtml(roomMeta.room.name || roomMeta.roomId)}` : ' вЂў Р’РЅРµ РєРѕРјРЅР°С‚С‹'}
                        </div>
                    </div>
                    <div style="display:flex; gap:8px; flex-wrap:wrap;">
                        <button class="secondary-btn admin-load-user-btn" data-uid="${uid}" style="width:auto; padding:8px 12px;">РћС‚РєСЂС‹С‚СЊ</button>
                        <button class="secondary-btn admin-force-leave-btn" data-uid="${uid}" style="width:auto; padding:8px 12px;" ${roomMeta ? '' : 'disabled'}>РљРёРє РёР· РєРѕРјРЅР°С‚С‹</button>
                        <button class="danger-btn admin-force-logout-btn" data-uid="${uid}" style="width:auto; padding:8px 12px;">Р’С‹РіРЅР°С‚СЊ</button>
                    </div>
                </div>
            `;
        }).join('');
    ...
    static async loadUserEditor(uid) {
        if (!this.requireAdmin()) return;
        if (!uid) return;
        if (!(await this.checkModRestrictionsForTarget(uid))) return; // Р—Р°С‰РёС‚Р° РЎРѕР·РґР°С‚РµР»СЏ
        ...
        editor.innerHTML = `
            ...
            <div style="font-size:12px; color:var(--text-muted); margin-top:-6px;">РљРѕРјРЅР°С‚Р°: ${roomMeta ? Utils.escapeHtml(roomMeta.room.name || roomMeta.roomId) : 'РЅРµ РЅР°С…РѕРґРёС‚СЃСЏ РІ РєРѕРјРЅР°С‚Рµ'}</div>
            <input type="text" id="admin-edit-name" placeholder="РРјСЏ" value="${Utils.escapeHtml(profile.name || '')}">
            <input type="text" id="admin-edit-username" placeholder="ID" value="${Utils.escapeHtml(profile.username || '')}">
            <input type="text" id="admin-edit-avatar" placeholder="URL Р°РІР°С‚Р°СЂРєРё" value="${Utils.escapeHtml(profile.avatar || '')}">
            <textarea id="admin-edit-bio" rows="4" placeholder="РћРїРёСЃР°РЅРёРµ">${Utils.escapeHtml(profile.bio || '')}</textarea>
            <div style="font-size:12px; color:var(--text-muted);">Email: ${Utils.escapeHtml(profile.email || 'РЅРµ СѓРєР°Р·Р°РЅ')}</div>
            <div style="display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:8px;">
                <button class="primary-btn" id="btn-admin-save-user">РЎРѕС…СЂР°РЅРёС‚СЊ РёР·РјРµРЅРµРЅРёСЏ</button>
                <button class="secondary-btn" id="btn-admin-reset-user">РћР±РЅСѓР»РёС‚СЊ РїСЂРѕС„РёР»СЊ</button>
                <button class="secondary-btn" id="btn-admin-force-leave-current">РљРёРєРЅСѓС‚СЊ РёР· РєРѕРјРЅР°С‚С‹</button>
                <button class="danger-btn" id="btn-admin-force-logout-current">Р¤РѕСЂСЃ-РІС‹С…РѕРґ</button>
            </div>
        `;
    ...
    static async findUser() {
        ...
        if (!rawValue) return Utils.toast('Р’РІРµРґРёС‚Рµ @id РёР»Рё uid', 'error');
        ...
        if (!usernameSnap.exists()) return Utils.toast('РџРѕР»СЊР·РѕРІР°С‚РµР»СЊ РЅРµ РЅР°Р№РґРµРЅ', 'error');
    ...
    static async saveUserProfile() {
        ...
        if (!uid) return Utils.toast('РЎРЅР°С‡Р°Р»Р° РІС‹Р±РµСЂРёС‚Рµ РїРѕР»СЊР·РѕРІР°С‚РµР»СЏ', 'error');
        if (!(await this.checkModRestrictionsForTarget(uid))) return; // Р—Р°С‰РёС‚Р° РЎРѕР·РґР°С‚РµР»СЏ

        const profileSnap = await get(ref(db, `users/${uid}/profile`));
        if (!profileSnap.exists()) return Utils.toast('РџСЂРѕС„РёР»СЊ РїРѕР»СЊР·РѕРІР°С‚РµР»СЏ РЅРµ РЅР°Р№РґРµРЅ', 'error');
        ...
        if (!name || !username) return Utils.toast('РРјСЏ Рё ID РѕР±СЏР·Р°С‚РµР»СЊРЅС‹', 'error');
        if (!/^[a-z0-9_]{3,15}$/.test(username)) return Utils.toast('ID: 3-15 СЃРёРјРІРѕР»РѕРІ, a-z, 0-9, _', 'error');

        const developerUid = await this.getDeveloperUid();
        const isCreatorTarget = Boolean(developerUid && uid === developerUid);

        if (username === 'developer' && !isCreatorTarget) return Utils.toast('ID developer Р·Р°СЂРµР·РµСЂРІРёСЂРѕРІР°РЅ', 'error');
        if (isCreatorTarget && username !== oldProfile.username) return Utils.toast('ID РЎРѕР·РґР°С‚РµР»СЏ РЅРµР»СЊР·СЏ РёР·РјРµРЅРёС‚СЊ', 'error');

        const updates = {};
        if (username !== oldProfile.username) {
            const usernameSnap = await get(ref(db, `usernames/${username}`));
            if (usernameSnap.exists() && usernameSnap.val() !== uid) return Utils.toast('Р­С‚РѕС‚ ID СѓР¶Рµ Р·Р°РЅСЏС‚', 'error');
            if (oldProfile.username) updates[`usernames/${oldProfile.username}`] = null;
            updates[`usernames/${username}`] = uid;
        }
        ...
        Utils.toast('РџСЂРѕС„РёР»СЊ РїРѕР»СЊР·РѕРІР°С‚РµР»СЏ РѕР±РЅРѕРІР»С‘РЅ');
    ...
    static async resetUserProfile() {
        ...
        if (!confirm('РћР±РЅСѓР»РёС‚СЊ РїСЂРѕС„РёР»СЊ РїРѕР»СЊР·РѕРІР°С‚РµР»СЏ?')) return;
        ...
        const oldProfile = profileSnap.val() || {};
        const developerUid = await this.getDeveloperUid();
        if (developerUid && uid === developerUid) return Utils.toast('РџСЂРѕС„РёР»СЊ РЎРѕР·РґР°С‚РµР»СЏ РЅРµР»СЊР·СЏ РѕР±РЅСѓР»РёС‚СЊ', 'error');
        ...
        const nextProfile = {
            ...oldProfile,
            name: 'РџСЂРѕС„РёР»СЊ СЃР±СЂРѕС€РµРЅ',
        ...
        await update(ref(db), updates);
        AppState.usersCache.set(uid, nextProfile);
        Utils.toast('РџСЂРѕС„РёР»СЊ РїРѕР»СЊР·РѕРІР°С‚РµР»СЏ РѕР±РЅСѓР»С‘РЅ');
        await this.loadUserEditor(uid);
        this.renderIfOpen();
    }
    ...
    static async sendAnnouncement() {
        ...
        if (!text) return Utils.toast('Р’РІРµРґРёС‚Рµ С‚РµРєСЃС‚ РѕРїРѕРІРµС‰РµРЅРёСЏ', 'error');
        ...
        Utils.toast('Р“Р»РѕР±Р°Р»СЊРЅРѕРµ РѕРїРѕРІРµС‰РµРЅРёРµ РѕС‚РїСЂР°РІР»РµРЅРѕ');
    }

    static async clearAnnouncement() {
        if (!this.requireAdmin()) return;
        await remove(ref(db, 'admin/global-announcement'));
        Utils.toast('Р“Р»РѕР±Р°Р»СЊРЅРѕРµ РѕРїРѕРІРµС‰РµРЅРёРµ РѕС‡РёС‰РµРЅРѕ');
    }

    static async deleteRoom(roomId) {
        if (!this.requireAdmin()) return;
        if (!(await this.checkModRestrictionsForRoom(roomId))) return; // Р—Р°С‰РёС‚Р° РєРѕРјРЅР°С‚ РЎРѕР·РґР°С‚РµР»СЏ

        const roomData = AppState.roomsCache.get(roomId);
        if (!roomData) return Utils.toast('РљРѕРјРЅР°С‚Р° СѓР¶Рµ СѓРґР°Р»РµРЅР°', 'error');
        if (!confirm(`Р—Р°РєСЂС‹С‚СЊ РєРѕРјРЅР°С‚Сѓ "${roomData.name || roomId}"?`)) return;
        ...
        this.renderIfOpen();
        Utils.toast('РљРѕРјРЅР°С‚Р° СѓРґР°Р»РµРЅР°');
    }

    static async deleteAllRooms() {
        if (!this.requireAdmin()) return;
        if (!confirm('РЈРґР°Р»РёС‚СЊ РІРѕРѕР±С‰Рµ РІСЃРµ РєРѕРјРЅР°С‚С‹? Р­С‚Рѕ РґРµР№СЃС‚РІРёРµ РЅРµРѕР±СЂР°С‚РёРјРѕ.')) return;
        ...
        for (const [roomId, room] of AppState.roomsCache.entries()) {
            // РњРѕРґРµСЂР°С‚РѕСЂС‹ РїСЂРѕРїСѓСЃРєР°СЋС‚ РєРѕРјРЅР°С‚С‹ РЎРѕР·РґР°С‚РµР»СЏ РїСЂРё РјР°СЃСЃ-СѓРґР°Р»РµРЅРёРё
        ...
        RoomManager.updateRoomsDOM();
        this.renderIfOpen();
        Utils.toast(`РЈРґР°Р»РµРЅРѕ РєРѕРјРЅР°С‚: ${deletedCount}`);
    }
    ...
    static async purgeEmptyRooms() {
        ...
        const emptyRoomIds = Array.from(AppState.roomsCache.entries())
            .filter(([, room]) => {
                ...
                // Р—Р°С‰РёС‚Р° РєРѕРјРЅР°С‚, СЃРѕР·РґР°РЅРЅС‹С… СЂР°Р·СЂР°Р±РѕС‚С‡РёРєРѕРј, РѕС‚ РјРѕРґРµСЂР°С‚РѕСЂРѕРІ
                if (isModOnly && room.hostId === devUid) return false; 
                return true;
            })
            .map(([roomId]) => roomId);

        if (!emptyRoomIds.length) return Utils.toast('Р”РѕСЃС‚СѓРїРЅС‹С… РґР»СЏ СѓРґР°Р»РµРЅРёСЏ РїСѓСЃС‚С‹С… РєРѕРјРЅР°С‚ РЅРµС‚');
        ...
        RoomManager.updateRoomsDOM();
        this.renderIfOpen();
        Utils.toast(`РЈРґР°Р»РµРЅРѕ РїСѓСЃС‚С‹С… РєРѕРјРЅР°С‚: ${emptyRoomIds.length}`);
    }

    static async clearDirectMessages() {
        if (!this.requireAdmin()) return;
        if (!this.isCurrentUserCreator()) return Utils.toast('РўРѕР»СЊРєРѕ РЎРѕР·РґР°С‚РµР»СЊ РјРѕР¶РµС‚ СѓРґР°Р»СЏС‚СЊ РІСЃРµ Р›РЎ', 'error');

        if (!confirm('РЈРґР°Р»РёС‚СЊ РІРѕРѕР±С‰Рµ РІСЃРµ Р»РёС‡РЅС‹Рµ СЃРѕРѕР±С‰РµРЅРёСЏ?')) return;

        await remove(ref(db, 'direct-messages'));
        this.renderIfOpen();
        Utils.toast('Р’СЃРµ Р»РёС‡РЅС‹Рµ СЃРѕРѕР±С‰РµРЅРёСЏ СѓРґР°Р»РµРЅС‹');
    }

    static async toggleRoomCreationLock() {
        if (!this.requireAdmin()) return;
        if (!this.isCurrentUserCreator()) return Utils.toast('РўРѕР»СЊРєРѕ РЎРѕР·РґР°С‚РµР»СЊ РјРѕР¶РµС‚ Р±Р»РѕРєРёСЂРѕРІР°С‚СЊ СЃРѕР·РґР°РЅРёРµ РєРѕРјРЅР°С‚', 'error');
        ...
        RoomManager.applyCreateRoomAvailability();
        this.renderIfOpen();
        Utils.toast(nextValue ? 'РЎРѕР·РґР°РЅРёРµ РєРѕРјРЅР°С‚ Р·Р°Р±Р»РѕРєРёСЂРѕРІР°РЅРѕ' : 'РЎРѕР·РґР°РЅРёРµ РєРѕРјРЅР°С‚ СЂР°Р·Р±Р»РѕРєРёСЂРѕРІР°РЅРѕ');
    }
    ...
    static async forceSignOut(uid) {
        ...
        if (!confirm(`РџСЂРёРЅСѓРґРёС‚РµР»СЊРЅРѕ Р·Р°РІРµСЂС€РёС‚СЊ СЃРµСЃСЃРёСЋ РїРѕР»СЊР·РѕРІР°С‚РµР»СЏ ${uid}?`)) return;
        ...
        Utils.toast('РљРѕРјР°РЅРґР° РЅР° С„РѕСЂСЃ-РІС‹С…РѕРґ РѕС‚РїСЂР°РІР»РµРЅР°');
    }
    ...
    static async forceLeaveRoom(uid) {
        ...
        const roomMeta = this.getCurrentRoomForUid(uid);
        if (!roomMeta) return Utils.toast('РџРѕР»СЊР·РѕРІР°С‚РµР»СЊ СЃРµР№С‡Р°СЃ РЅРµ РЅР°С…РѕРґРёС‚СЃСЏ РІ РєРѕРјРЅР°С‚Рµ', 'error');
        
        if (!(await this.checkModRestrictionsForRoom(roomMeta.roomId))) return; // Р”РѕРї. Р·Р°С‰РёС‚Р° РєРѕРјРЅР°С‚С‹

        if (!confirm(`РЈРґР°Р»РёС‚СЊ РїРѕР»СЊР·РѕРІР°С‚РµР»СЏ ${uid} РёР· РєРѕРјРЅР°С‚С‹ "${roomMeta.room.name || roomMeta.roomId}"?`)) return;
        ...
        Utils.toast('РџРѕР»СЊР·РѕРІР°С‚РµР»СЊ СѓРґР°Р»С‘РЅ РёР· РєРѕРјРЅР°С‚С‹');
        this.renderIfOpen();
    }
...
// ============================================================================
// 6. РџРћР›РќРђРЇ РЎРРЎРўР•РњРђ РљРћРњРќРђРў Р РџР РђР’
// ============================================================================
...
class RoomManager {
    ...
    static applyCreateRoomAvailability() {
        ...
        btn.title = blockedForUser ? 'РЎРѕР·РґР°РЅРёРµ РєРѕРјРЅР°С‚ РІСЂРµРјРµРЅРЅРѕ РѕС‚РєР»СЋС‡РµРЅРѕ Р°РґРјРёРЅРёСЃС‚СЂР°С‚РѕСЂРѕРј' : '';
    }
    ...
    static updateRoomsDOM() {
        ...
        AppState.roomsCache.forEach((room, id) => {
            ...
            card.querySelector('.rm-host').innerText = `РҐРѕСЃС‚: ${room.hostName || 'РќРµРёР·РІРµСЃС‚РЅРѕ'}`;
            card.querySelector('.rm-count').innerText = `рџ‘Ґ ${membersCount}`;
            count++;
        });
        ...
        if (count === 0 && !Utils.$('empty-rooms-msg')) {
            ...
            msg.innerText = search ? 'РќРёС‡РµРіРѕ РЅРµ РЅР°Р№РґРµРЅРѕ' : 'РќРµС‚ Р°РєС‚РёРІРЅС‹С… РєРѕРјРЅР°С‚';
    ...
    static openRoomModal(roomId = null) {
        if (!roomId && AppState.admin.settings.roomCreationBlocked && !AdminPanel.isCurrentUserAdmin()) {
            return Utils.toast('РЎРѕР·РґР°РЅРёРµ РєРѕРјРЅР°С‚ РІСЂРµРјРµРЅРЅРѕ РѕС‚РєР»СЋС‡РµРЅРѕ Р°РґРјРёРЅРёСЃС‚СЂР°С‚РѕСЂРѕРј', 'error');
        }

        const modal = Utils.$('modal-room');
        const isEdit = !!roomId;
        Utils.$('room-modal-title').innerText = isEdit ? 'РќР°СЃС‚СЂРѕР№РєРё РєРѕРјРЅР°С‚С‹' : 'РЎРѕР·РґР°С‚СЊ РєРѕРјРЅР°С‚Сѓ';
        ...
        if (isEdit) {
            ...
            Utils.$('btn-delete-room').onclick = async () => {
                if(confirm('РўРѕС‡РЅРѕ СѓРґР°Р»РёС‚СЊ РєРѕРјРЅР°С‚Сѓ РЅР°РІСЃРµРіРґР°?')) {
    ...
    static async saveRoom() {
        ...
        if (!name) return Utils.toast('РќР°Р·РІР°РЅРёРµ РЅРµ РјРѕР¶РµС‚ Р±С‹С‚СЊ РїСѓСЃС‚С‹Рј', 'error');
        if (isPrivate && password.length < 4 && !roomId) return Utils.toast('РџР°СЂРѕР»СЊ РјРёРЅРёРјСѓРј 4 СЃРёРјРІРѕР»Р°', 'error');

        Utils.$('btn-save-room').disabled = true;
        try {
            const roomData = { name, videoUrl, isPrivate, hostId: AppState.currentUser.uid, hostName: AppState.currentUser.displayName || 'РҐРѕСЃС‚', updatedAt: Date.now() };
            ...
            if (roomId) {
                ...
                await update(ref(db, `rooms/${roomId}`), roomData); Utils.toast('РќР°СЃС‚СЂРѕР№РєРё СЃРѕС…СЂР°РЅРµРЅС‹');
            } else {
                roomData.createdAt = Date.now(); const newRef = push(ref(db, 'rooms')); await set(newRef, roomData); Utils.toast('РљРѕРјРЅР°С‚Р° СЃРѕР·РґР°РЅР°');
                this.enterRoomFinal(newRef.key, roomData);
            }
            Utils.$('modal-room').classList.remove('active');
        } catch (e) { Utils.toast('РћС€РёР±РєР° СЃРѕС…СЂР°РЅРµРЅРёСЏ', 'error'); } 
    ...
    static async attemptJoinRoom(roomId, roomData) {
        if (roomData.isPrivate && roomData.hostId !== AppState.currentUser.uid) {
            ...
            Utils.$('btn-submit-password').onclick = async () => {
                ...
                else Utils.toast('РќРµРІРµСЂРЅС‹Р№ РїР°СЂРѕР»СЊ', 'error');
            };
        } else {
    ...
    static enterRoomFinal(roomId, roomData) {
        ...
        sessionStorage.setItem('cow:lastRoomId', roomId);
        ...
        if (vid) {
            ...
            vid.preload = 'auto';
            vid.onerror = () => Utils.toast('РџР»РµРµСЂ РЅРµ СЃРјРѕРі Р·Р°РіСЂСѓР·РёС‚СЊ РІРёРґРµРѕ. РќСѓР¶РЅР° РїСЂСЏРјР°СЏ СЃСЃС‹Р»РєР° РЅР° РјРµРґРёР°С„Р°Р№Р».', 'error');
        }
        ...
        if (!shareBtn) {
            ...
            shareBtn.innerText = 'РџРѕРґРµР»РёС‚СЊСЃСЏ';
            Utils.$('btn-room-settings').parentNode.appendChild(shareBtn);
        }
        shareBtn.onclick = () => {
            Utils.$('tab-users-btn').click();
            Utils.toast('РќР°Р¶РјРёС‚Рµ "РџСЂРёРіР»Р°СЃРёС‚СЊ" СЂСЏРґРѕРј СЃ РґСЂСѓРіРѕРј РІ СЃРїРёСЃРєРµ', 'info');
        };
        ...
        Utils.$('chat-messages').innerHTML = '<div class="sys-msg">Р’С‹ РІРѕС€Р»Рё РІ РєРѕРјРЅР°С‚Сѓ</div>';
    ...
    static initRoomServicesFinal(roomId) {
        ...
        const cUnsub = onChildAdded(chatRef, (snap) => {
            ...
            line.querySelectorAll('.timecode-btn').forEach(btn => {
                btn.onclick = () => {
                    if (!this.hasPerm('player')) return Utils.toast('РќРµС‚ РїСЂР°РІ РЅР° СѓРїСЂР°РІР»РµРЅРёРµ РїР»РµРµСЂРѕРј', 'error');
    ...
    static rerenderUsersList() {
        ...
        if (AppState.currentUser) {
            get(ref(db, `users/${AppState.currentUser.uid}/friends`)).then(snap => {
                ...
                if (friendsIds.length > 0) {
                    let inviteHtml = `<div style="font-size:11px; color:var(--text-muted); margin: 10px 0 5px; text-transform:uppercase;">Р”СЂСѓР·СЊСЏ РІРЅРµ РєРѕРјРЅР°С‚С‹</div>`;
                    friendsIds.forEach(fid => {
                        inviteHtml += `
                            <div class="user-item" style="background: rgba(46,213,115,0.05); border: 1px solid rgba(46,213,115,0.2);">
                                <div class="user-main"><span class="user-name" id="inv-name-${fid}">Р—Р°РіСЂСѓР·РєР°...</span></div>
                                <button class="primary-btn" style="width:auto; padding:4px 8px; font-size:11px;" onclick="DirectMessages.sendRoomInvite('${fid}')">РџСЂРёРіР»Р°СЃРёС‚СЊ</button>
                            </div>
                        `;
                    ...
                    container.innerHTML += inviteHtml + `<div style="font-size:11px; color:var(--text-muted); margin: 15px 0 5px; text-transform:uppercase;">Р’ РєРѕРјРЅР°С‚Рµ</div>`;
                }
                renderRoomUsers();
            });
        } else {
        ...
        function renderRoomUsers() {
            ...
            ids.forEach(uid => {
                ...
                // Р РµРЅРґРµСЂ Р±РµР№РґР¶Р° СЂРѕР»РµР№ РґР»СЏ СЃРїРёСЃРєР° РєРѕРјРЅР°С‚С‹
                const profile = AppState.usersCache.get(uid) || {};
                ...
                if (isLocal) html += `<span class="you-label">(Р’С‹)</span>`;
                html += `</div>`;

                html += `<div class="user-card-actions">`;
                if (!isLocal) {
                    html += `<button class="dm-btn" data-uid="${uid}">рџ’¬</button>`;
                    html += `<button class="add-friend-btn" data-uid="${uid}">+Р”СЂСѓРі</button>`;
                }
                html += `</div>`;
                ...
                if (AppState.isHost && !isLocal) {
                    ...
                    html += `
                        <div class="perm-controls">
                            <label><input type="checkbox" class="p-toggle" data-uid="${uid}" data-p="chat" ${perms.chat?'checked':''}> Р§Р°С‚</label>
                            <label><input type="checkbox" class="p-toggle" data-uid="${uid}" data-p="voice" ${perms.voice?'checked':''}> РњРёРєСЂРѕС„РѕРЅ</label>
                            <label><input type="checkbox" class="p-toggle" data-uid="${uid}" data-p="player" ${perms.player?'checked':''}> РџР»РµРµСЂ</label>
                            <label><input type="checkbox" class="p-toggle" data-uid="${uid}" data-p="reactions" ${perms.reactions?'checked':''}> Р РµР°РєС†РёРё</label>
                        </div>
                    `;
                }
                html += `</div>`;
...
// ============================================================================
// 6. WEBRTC MESH SYSTEM (Р’РѕСЃСЃС‚Р°РЅРѕРІР»РµРЅРЅР°СЏ РЅР°РґРµР¶РЅР°СЏ РІРµСЂСЃРёСЏ)
// ============================================================================
...
class RTCManager {
    ...
    static async toggleMic(forceOff = false) {
        ...
        if (this.isMicActive || forceOff) {
            ...
            if (!forceOff) Utils.toast('РњРёРєСЂРѕС„РѕРЅ РІС‹РєР»СЋС‡РµРЅ');
        } else {
            if (!RoomManager.hasPerm('voice')) return Utils.toast('Р’Р°Рј Р·Р°РїСЂРµС‰РµРЅРѕ РіРѕРІРѕСЂРёС‚СЊ', 'error');
            try {
                ...
                await this.writeParticipantState();
                await this.handleParticipants(AppState.rtc.voiceParticipantsCache || {});
                Utils.toast('РњРёРєСЂРѕС„РѕРЅ РІРєР»СЋС‡РµРЅ');
            } catch (e) {
                btn.style.opacity = '1';
                this.isMicActive = false;
                btn.classList.remove('active');
                Utils.toast('РќРµС‚ РґРѕСЃС‚СѓРїР° Рє РјРёРєСЂРѕС„РѕРЅСѓ', 'error');
            }
        }
    }
...
// ============================================================================
// 10. Р—РђРџРЈРЎРљ РџР РР›РћР–Р•РќРРЇ
// ============================================================================


export { AuthManager, ProfileManager, FriendsManager, DirectMessages, AdminPanel, RoomManager, RTCManager };
