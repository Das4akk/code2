// Telegram UI Clone logic for Direct Messages
const { ref, get, set, push, update, remove, onValue, off } = window.firebaseDatabase;
...
.dm-sidebar-header input.tg-search {
    width: 100%;
    background: #242f3d;
    border: none;
    padding: 8px 15px;
...
const oldOpenChat = DirectMessages.openChat;
DirectMessages.openChat = function(targetUid, targetName) {
    oldOpenChat.call(this, targetUid, targetName);
    
    // Inject Telegram Input UI
    const controlsWrap = document.querySelector("#dm-messages").nextElementSibling;
    if (controlsWrap && !document.getElementById("tg-bar-injected")) {
        ...
        standardInputs.forEach(el => {
            if(el.id !== "dm-media-picker" && el.className !== "tg-input-area" && el.parentElement.className !== "tg-input-area") {
